...
class SplitBatchNorm2d(torch.nn.BatchNorm2d):
    ...
    def __init__(self, num_features, eps=1e-5, momentum=0.1, affine=True,
                 track_running_stats=True, num_splits=2, batch=64):
...
#            print('batch', self.batch)
            if self.batch != split_size * self.num_splits:
                split_input = input
                noise = True
            else:
                noise = False
 ...
 #           print('split_input[0]', len(split_input))
            if noise:
                for i, a in enumerate(self.aux_bn):
  #                  print('jjjj',i)
                    x = [a(split_input)]
                # x = [self.aux_bn(split_input)]
            else:
...
def convert_splitbn_model(module, num_splits=2, batch=64):
    """
    ...
    if isinstance(module, torch.nn.modules.batchnorm._BatchNorm):
        mod = SplitBatchNorm2d(
            ...
            module.track_running_stats, num_splits=num_splits, batch=batch)
    ...
    for name, child in module.named_children():
        mod.add_module(name, convert_splitbn_model(child, num_splits=num_splits, batch=batch))
    del module
    return mod
